...
export const CreatorQuestionCard: React.FC<CreatorQuestionCardProps> = ({
  ...
  useEffect(() => {
    // Reset local answer draft when question changes
    setAnswer('');
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <textarea
            ...
            disabled={isLoading}
            spellCheck={true}
            placeholder={isProposal && selectedProposalAction === 'ACCEPT' ? 'Using accepted direction (or add fine details)...' : translated.inputPlaceholder}
            className="w-full px-5 py-4 rounded-2xl bg-black/50 border border-white/15 focus:border-[#FF6500] focus:ring-2 focus:ring-[#FF6500]/20 text-white text-sm sm:text-base placeholder-white/25 outline-none transition-all resize-none leading-relaxed"
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={isLoading || (!answer.trim() && !(isProposal && selectedProposalAction === 'ACCEPT'))}
      ...
      {showCraftDetails && (
        <div className="mt-4 pt-4 border-t border-white/5 space-y-2 text-xs text-white/60 font-sans">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/80">{translated.behindTheScenes.deficiencyDescription}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="font-mono text-white/40 uppercase text-[10px]">Creative Skill:</span>
